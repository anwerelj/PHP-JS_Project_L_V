...
<div class="form-container">
    ...
    <?php if(isset($_POST['nom'])) {
        ...
        if(empty($nom) || empty($adresse) || empty($telephone) || empty($email) || empty($password)) {
            echo '<div class="alert alert-error">Tous les champs sont obligatoires.</div>';
        } elseif(!filter_var($email, FILTER_VALIDATE_EMAIL)) {
            echo '<div class="alert alert-error">Format d\'email invalide.</div>';
        } elseif(strlen($password) < 6) {
            echo '<div class="alert alert-error">Le mot de passe doit contenir au moins 6 caractères.</div>';
        } elseif($password !== $confirm) {
            echo '<div class="alert alert-error">Les mots de passe ne correspondent pas.</div>';
        } else {
            ...
            if($stmt->fetch(PDO::FETCH_OBJ)) {
                ...
                $stmt   = $db->prepare("INSERT INTO Clients (Nom, Adresse, Numero_telephone, Email, Mot_de_passe) VALUES (:nom, :adresse, :tel, :email, :mdp)");
                $stmt->execute([':nom'=>$nom, ':adresse'=>$adresse, ':tel'=>$telephone, ':email'=>$email, ':mdp'=>$hashed]);
                echo '<div class="alert alert-success">Inscription réussie !</div>';
                echo '<p style="text-align:center;"><a href="connexion.php" class="btn btn-primary">Se connecter</a></p>';
            }
        }
    } else { ?>
    ...
    <form method="POST" action="">
        ...
        <div class="form-group">
            <label>Adresse *</label>
            <textarea name="adresse" rows="2" required></textarea>
        </div>
        <div class="form-group">
    ...
    <p style="text-align:center;margin-top:1rem;">
        Déjà inscrit ? <a href="connexion.php" style="color:#14b8a6;">Connectez-vous</a>
    </p>
